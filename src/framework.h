...
//---------------------------
struct mat4Global { // row-major matrix 4x4
//---------------------------
	vec4 rows[4];
public:
	mat4Global() {}
	mat4Global(float m00, float m01, float m02, float m03,
	...
	mat4Global(vec4 it, vec4 jt, vec4 kt, vec4 ot) {
...
inline vec4 operator*(const vec4& v, const mat4Global& mat) {
	return v[0] * mat[0] + v[1] * mat[1] + v[2] * mat[2] + v[3] * mat[3];
}

inline mat4Global operator*(const mat4Global& left, const mat4Global& right) {
	mat4Global result;
	for (int i = 0; i < 4; i++) result.rows[i] = left.rows[i] * right;
	return result;
}

inline mat4Global TranslateMatrix(vec3 t) {
	return mat4Global(vec4(1,   0,   0,   0),
			    vec4(0,   1,   0,   0),
				vec4(0,   0,   1,   0),
				vec4(t.x, t.y, t.z, 1));
}

inline mat4Global ScaleMatrix(vec3 s) {
	return mat4Global(vec4(s.x, 0,   0,   0),
			    vec4(0,   s.y, 0,   0),
				vec4(0,   0,   s.z, 0),
				vec4(0,   0,   0,   1));
}

inline mat4Global RotationMatrix(float angle, vec3 w) {
	float c = cosf(angle), s = sinf(angle);
	w = normalize(w);
	return mat4Global(vec4(c * (1 - w.x*w.x) + w.x*w.x, w.x*w.y*(1 - c) + w.z*s, w.x*w.z*(1 - c) - w.y*s, 0),
...
public:
	...
	TextureGlobal() { textureId = 0; }

	TextureGlobal(std::string pathname, bool transparent = false) {
		textureId = 0;
		create(pathname, transparent);
	}

	TextureGlobal(int width, int height, const std::vector<vec4>& image, int sampling = GL_LINEAR) {
		textureId = 0;
		create(width, height, image, sampling);
	}

	TextureGlobal(const TextureGlobal& texture) {
		printf("\nError: Texture resource is not copied on GPU!!!\n");
	}

	void operator=(const TextureGlobal& texture) {
		printf("\nError: Texture resource is not copied on GPU!!!\n");
	}
